...
- [x] CRUD操作関数の実装
  - [x] `addNode(node: Node)`
  - [x] `updateNode(id: string, updates: Partial<Node>)`
  - [x] `deleteNode(id: string)`
  - [x] `addLink(link: Link)`
  - [x] `deleteLink(id: string)`
- [x] ユニークID生成ユーティリティ（nanoid等）
...
- [x] ノード追加機能
  - [x] 「+」ボタンUI
  - [x] デフォルト位置での新規ノード作成
- [x] ノード編集パネル（モーダル/サイドパネル）
  - [x] タイトル入力フィールド
  - [x] URL入力フィールド
  - [x] カテゴリ選択（ラジオボタン/セレクト）
  - [x] 種類選択（ドロップダウン）
  - [x] 説明文テキストエリア
  - [x] カスタムメトリクス管理UI
    - [x] メトリクス追加ボタン
...
- [x] ノード削除機能
  - [x] 削除ボタン
  - [x] 確認ダイアログ
  - [ ] Deleteキーでの削除対応（Phase 10に延期）
- [x] ノードのドラッグ&ドロップ移動（Phase 7で実装）
  - [x] Shift+ドラッグでの実装
  - [x] 移動中の視覚フィードバック
  - [x] 位置情報の更新
...
- [x] リンク（エッジ）の3D描画
  ...
  - [x] 矢印の表示（カスタムコーン）
  - [x] リンクの方向性の可視化
- [ ] リンク作成機能（方法A: ドラッグ&ドロップ）（実装せず）
  - [ ] ノード端からのドラッグ開始検出
  - [ ] ドラッグ中の一時的な線の表示
  - [ ] ターゲットノードへのドロップ検出
  - [ ] リンク確定処理
- [x] リンク作成機能（方法B: クリック選択）
  - [x] 「リンク追加モード」のトグル
  - [x] ソースノード選択
  - [x] ターゲットノード選択
  - [x] リンク作成完了
...
### タスク
- [x] 手動配置機能の完成
  - [x] ドラッグ&ドロップでの3D空間内移動（Shift+ドラッグ）
  - [x] X, Z軸方向への自由移動（Y軸固定）
- [x] 自動レイアウトアルゴリズムの実装
  - [x] 階層型レイアウトの実装
    - [x] リンク構造に基づく階層分析（BFS）
    - [x] カテゴリに基づくZ軸配置（traffic: 負, cashpoint: 正）
    - [x] レベルに基づくX軸配置
    - [x] 同レベル内でのY軸均等配置
  - [ ] 力学ベースレイアウト（今回は実装せず）
  - [x] レイアウト実行ボタンの実装
  - [x] アニメーション遷移（React Three Fiberの自然な遷移）
- [x] リセットビュー機能（Phase 6で実装済み）
  - [x] カメラ位置の初期化
  - [x] カメラターゲットのリセット
  - [x] OrbitControls.reset()使用
- [x] オートフィット機能（Phase 6で実装済み）
  - [x] 全ノードのバウンディングボックス計算
  - [x] カメラ位置の自動調整
  - [x] 全ノードが画面内に収まる表示
