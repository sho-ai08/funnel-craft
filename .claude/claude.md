...
### 実装メモ
- **ファイル構成**:
  - `src/utils/validation.ts`: バリデーション関数群（新規作成）
  - `src/components/ui/ConfirmDialog.tsx`: 確認ダイアログコンポーネント（新規作成）
  - `src/components/layout/Header.tsx`: 保存・読み込み機能の実装
  - `src/store/useStore.ts`: loadProject関数のUIState初期化修正
- **バリデーション機能**:
  - `validateNode`: 個別ノードデータの検証
  - `validateLink`: 個別リンクデータの検証
  - `validateProjectData`: プロジェクト全体のデータ検証
  - `checkDuplicateIds`: ID重複チェック
  - リンクの整合性チェック（存在しないノードへの参照検出）
- **ConfirmDialogコンポーネント**:
  - 再利用可能な確認ダイアログ
  - 3つのタイプ: warning, info, danger
  - カスタマイズ可能なメッセージとボタンラベル
  - ダークテーマのモダンUI
- **保存機能**:
  - バージョン情報付きJSON出力（version: "1.0"）
  - タイムスタンプ自動付与
  - ファイル名に日時を含める
  - エラーハンドリング実装
- **読み込み機能**:
  ...
  - 複数のエラーケースに対応:
    ...
    - リンクの参照エラー
- **loadProject関数の改良**:
  - すべてのUIState項目を正しく初期化
  - resetViewTrigger, autoFitTrigger, isSidePanelOpenを含む
...
- ✅ **Phase 7完了: レイアウト・ビュー機能**
  ...
  - 開発サーバー動作確認
- ✅ **Phase 8完了: 保存・読み込み機能**
  - バリデーション関数群作成（validation.ts）
  - ConfirmDialogコンポーネント作成
  - Header コンポーネントに保存・読み込み機能実装
  - JSON保存機能（バージョン情報・タイムスタンプ付き）
  - JSON読み込み機能（包括的バリデーション付き）
  - 確認ダイアログ実装（データ統計表示）
  - エラーハンドリング強化
  - loadProject関数のUIState初期化修正
  - 開発サーバー動作確認
- 📝 次回: Phase 9（エクスポート機能）
